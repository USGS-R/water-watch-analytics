...
```
### Executive Summary
Waterwatch traffic is decreasing past couple years
most valuable categories in each watch - ToP, entrance/exits/etc narrative
...
Code can be found [here](https://github.com/USGS-R/water-watch-analytics).
...
c(sessions_plot, sessions_plot_int, users_plot) %<-% total_sessions_users_plot()
sessions_plot
users_plot
sessions_plot_int
```
...
State maps are the most popular, somewhat moreso than the national map.  
...
Groundwater Watch site pages receive by far the most traffic, followed by the landing page (which links to multiple national views), and state and national views.
...
Across all three Watches, state and site views are much more popular than national views by the summed unique pageviews metric. 

## Other dimensions to explore
We have the data available to explore many other dimensions of web traffic, if there are useful applications for them.  With methods similar to this report, we can look at proportions of USGS network traffic (this report does not distinguish between external and interal traffic), entry and exit pages, popular monitoring sites, geographic origination around certain events, and many others. 
